...
protected:
    ...
    void merge(ListNodePosi(T)&, int, List<T>&, ListNodePosi(T), Rank); //归并
...
public:
    ...
    void merge(List<T>& L) { merge(header->succ, _size, L, L.header->succ, L._size); } //全列    void sort( ListNodePosi<T>, Rank ); //列表区间排序
